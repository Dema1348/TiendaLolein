    ...
    <div class="cuerpo">
       ...
       <% if(sesion.getAttribute("usuario")==null){
        ...
        <div class="contenido">
            ...
            <div class="detalle">
                <h2>* SELECCIONA LA CANTIDAD PARA CONTINUAR *</h2>
                <% 
                    MantenedoraProductos productosWeb= new MantenedoraProductos();
                    Producto item= new Producto();
                    item =productosWeb.buscar(Integer.parseInt(request.getParameter("Id")));
                  
                    
                        %>
                        <article>
                        <img src="img/<%= item.getFoto() %>" alt="Producto">
                                
                        </article>
                        <div class="detalles">
                            <h3>ID: <%= item.getId() %> </h3>
                            <h3>Precio: $<%= item.getPrecio() %></h3>
                            <h3>Stok: <%= item.getStock() %></h3>
                            <h3>Detalle: <%= item.getDetalle() %></h3>  
                            <form action="controladorCarro.jsp" method="GET">
                                <label>Cantidad</label>
                                <input type="number" name="cantidad" min="1" max="<%= item.getStock() %>" required>
                                <input type="hidden" name="Id" value="<%= item.getId() %>" >
                                <br/>
                                <input class="addCarro" type="submit" value="enviar" name="addCarro">
                            </form>
                        </div>
